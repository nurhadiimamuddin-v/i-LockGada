        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table id="riwayatAmbilTable" class="display" style="width:100%">
                <thead>
                  <tr>
                    <th>No</th>
                    <th>Foto Barang</th>
                  ...
                  <tr v-for="(item, index) in ambilList" :key="item.id">
                    ...
                    <td>
                      <button class="btn" style="font-size: 15px; color: white; padding: 8px 12px; background-color: #152453;" @click="cetakSurat(item.id)">
                        <i class="mdi mdi-file-check"></i> Surat Barang diterima
                      </button>
                    </td>
                  </tr>
                </tbody>
...
const loadData = async () => {
  try {
    ...
    // Fetch rahin and users (pegawai) to map their NIKs
    ...
    ambilSnap.forEach(doc => {
      ...
      const pegawaiData = usersMap[data.nasabah_id] || {} // nasabah_id stores pegawai ID
